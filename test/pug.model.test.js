...
describe('Pug model', () => {
    ...
    describe('instance method: shortBio', () => {
        ...
        it('returns first sentence of bio', async () => {
            const cody = await Pug.create({
                name: 'Cody',
                biography:
                    'He is a pug. A cuddly pug. But also kind of a brat.',
            });
            ...
            const penny = await Pug.create({
                name: 'Penny',
                biography:
                    'Who is Penny the Pug? Only the most popular pug around!',
            });
    ...
    describe('class method: `findByCoffee`', () => {
        it('finds all pugs with the given favorite coffee', async () => {
            ...
            expect(latteLovers.some(pug => pug.name === 'Penny')).to.equal(
                true
            );

            expect(cortadoCourtiers.length).to.equal(1);
            expect(cortadoCourtiers.some(pug => pug.name === 'Doug')).to.equal(
                true
            );
        });
        ...
        it('finds pugs and eagerly loads their favorite coffee', async () => {
            ...
            expect(espressoEnthusiasts[0].favoriteCoffee.id).to.equal(
                espresso.id
            );
        });
    });
